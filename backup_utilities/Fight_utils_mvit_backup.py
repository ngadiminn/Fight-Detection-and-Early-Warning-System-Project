...
SEQUENCE_LENGTH = 16  # MViT biasanya menggunakan 16 frames
predicted_class_name = ""

# Define the transforms untuk MViT_v2_s
def transform_():
    """
    Transforms untuk MViT_v2_s
    MViT menggunakan input size 224x224 (berbeda dari MC3 yang 112x112)
    """
    transform = A.Compose([
        A.Resize(256, 256, always_apply=True),  # Resize lebih besar dulu
        A.CenterCrop(224, 224, always_apply=True),  # Center crop ke 224x224
        A.Normalize(
            mean=[0.45, 0.45, 0.45],  # Mean untuk MViT
            std=[0.225, 0.225, 0.225],  # Std untuk MViT
            always_apply=True
        )
    ])
    return transform
...
def frames_extraction(video_path, SEQUENCE_LENGTH):
    ...
    Args:
        ...
        SEQUENCE_LENGTH: The number of Frames we want.
    Returns:
...
def loadModel(modelPath):
    """
    Load MViT_v2_s model
    """
    PATH = modelPath
    
    # Load MViT_v2_s pretrained model
    model_ft = torchvision.models.video.mvit_v2_s(weights='DEFAULT')
    
    # Modifikasi classifier head untuk 2 kelas
    # MViT memiliki struktur head yang berbeda dari MC3
    num_ftrs = model_ft.head[1].in_features  # MViT menggunakan Sequential head
...
def create_fresh_mvit_model(num_classes=2, pretrained=True):
    """
    Create a fresh MViT_v2_s model for training
    """
    if pretrained:
    ...
    # Modifikasi head untuk jumlah kelas yang diinginkan
    num_ftrs = model.head[1].in_features
    model.head[1] = torch.nn.Linear(num_ftrs, num_classes)
...
def FightInference(video_path, model, SEQUENCE_LENGTH=16):
...
def FightInference_Time(video_path, model, SEQUENCE_LENGTH=16):
...
def predict_on_video(video_file_path, output_file_path, model, SEQUENCE_LENGTH, skip=2, showInfo=False):
    '''
    This function will perform action recognition on a video using the MViT model.
    Args:
