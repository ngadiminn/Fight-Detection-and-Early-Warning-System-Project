...
def tugas_ngirim_telegram():
    ...
    try:
        ...
        # 1. Catat waktu AWAL tepat sebelum menembak API Telegram
        start_time = time.time()
        
        response = requests.post(url, json=payload, timeout=5)
        
        # 2. Catat waktu AKHIR tepat setelah server Telegram memberikan respon sukses
        end_time = time.time()
        
        # 3. Hitung selisih latensi dalam milidetik (ms)
...
while True:
    ...
    if predicted_class_name == "fight": # BAHAYA (1)
        ...
        if status_terakhir != 1:
            # Mengambil dan mencetak timestamp pertama kali pengiriman bahaya ke ESP32
        ...
        # Cek apakah durasi sudah tembus 14 detik dan telegram belum terkirim
