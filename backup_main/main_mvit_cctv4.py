...
# KONFIGURASI MODEL
# ==========================================
MODEL_PATH = "mvit_fight_modelb8lr0001.pth"
SEQUENCE_LENGTH = 16
FRAME_SKIP = 2 
...
while True:
    ...
    # ==========================================
    # LOGIKA UTAMA (SISTEM SKENARIO ESP32 & TELEGRAM)
    # ==========================================
    if predicted_class_name == "fight": 
        if status_terakhir != 1:
            # Pertama kali banget AI nangkep berantem
            status_terakhir = 1
            waktu_mulai_bahaya = time.time()
            telegram_terkirim = False        
            
        # Hitung udah berapa lama mereka berantem
        ...
        # --- LOGIKA PENENTUAN SKENARIO ALARM BERDASARKAN WAKTU ---
        # Skenario 1: 0 - 4.99 detik (Strobo Only)
        # Skenario 2: 5 - 11.99 detik (Strobo + Sirine)
        # Skenario 3: > 12 detik (Strobo + Voice)
        ...
        # Kirim perintah ke ESP32 HANYA jika ada perubahan skenario
        # Biar jaringan WiFi nggak macet disepak API terus-terusan
        if status_esp_sekarang != target_skenario:
            print(f">>> Mengirim perintah Skenario {target_skenario} ke ESP32...")
            threading.Thread(target=tugas_ngirim_ke_esp, args=(target_skenario,)).start()
            status_esp_sekarang = target_skenario # Catat biar nggak dikirim ulang
        
        # --- LOGIKA TELEGRAM TETAP DI 14 DETIK ---
        ...
        if status_terakhir != 0:
            print(">>> Kondisi Aman. Mengirim Skenario 0 ke ESP32 & reset timer...")
            threading.Thread(target=tugas_ngirim_ke_esp, args=(0,)).start()
            
            status_terakhir = 0
            status_esp_sekarang = 0   # Reset ingatan skenario ESP
            waktu_mulai_bahaya = 0    
            telegram_terkirim = False 
