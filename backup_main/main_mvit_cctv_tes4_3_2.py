...
# KONFIGURASI ALAMAT
# ==========================================
RTSP_URL = "rtsp://admin:Artyago_23@192.168.1.19:554/cam/realmonitor?channel=1&subtype=1" 
...
# --- TAMBAHAN VARIABEL UNTUK BAB 4.3.2 ---
total_frame_diekstraksi = 0
total_waktu_ekstraksi = 0
print("\n--- MEMULAI PENGUKURAN KECEPATAN EKSTRAKSI FRAME (BAB 4.3.2) ---")

while True:
    # 1. Mulai stopwatch ekstraksi
    waktu_mulai_baca = time.time()
    
    success, img = cap.read()
    
    # 2. Hentikan stopwatch ekstraksi
    waktu_selesai_baca = time.time()
    
    if not success:
        time.sleep(0.1)
        continue

    # --- PERHITUNGAN LOGGING WAKTU ---
    durasi_per_frame = waktu_selesai_baca - waktu_mulai_baca
    total_waktu_ekstraksi += durasi_per_frame
    total_frame_diekstraksi += 1
    
    # Menampilkan data per 100 frame agar terminal tidak penuh (Biar kamu gampang nyatetnya)
    if total_frame_diekstraksi % 100 == 0:
        rata_rata_waktu = total_waktu_ekstraksi / 100
        fps_ekstraksi = 1 / rata_rata_waktu if rata_rata_waktu > 0 else 0
        
        print(f"[TESTING FPS] Total Frame: {total_frame_diekstraksi} | Rata-rata Waktu/Frame: {rata_rata_waktu:.4f} detik | Kecepatan: {fps_ekstraksi:.1f} FPS")
        
        # Reset counter waktu untuk perhitungan 100 frame berikutnya
        total_waktu_ekstraksi = 0
    ...
    if predicted_class_name == "fight": # BAHAYA (1)
        ...
        cv2.putText(display_img, f"BAHAYA (1) - Durasi: {int(durasi_bahaya)}s", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
        ...
        cv2.putText(display_img, "AMAN (0) - No Fight", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 3)
    else:
